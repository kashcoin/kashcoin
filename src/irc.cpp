...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KashcoinTEST2\r");
            Send(hSocket, "WHO #KashcoinTEST2\r");
        } else {
            // randomly join #Kashcoin00-#Kashcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Kashcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Kashcoin%02d\r", channel_number).c_str());
        }
