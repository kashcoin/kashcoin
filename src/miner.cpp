...
// Copyright (c) 2013 The Kashcoin developers
...
void ThreadStakeMiner(void* parg)
{
    ...
    try
    {
        ...
        do
        {
            ...
            if (fTrySync)
            {
                // Don't try mine blocks unless we're at the top of chain and have at least two p2p connections, and POS is active.
                fTrySync = false;
                if (vNodes.size() < 2 || nBestHeight < GetNumBlocksOfPeers() || nBestHeight < FIRST_POS_BLOCK-1)
                {
                    Sleep(1000);
                    continue;
                }
